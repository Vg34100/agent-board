...
use crate::features::kanban::services::{
    create_task_handler, delete_task, cancel_task,
    load_project_data, create_project_update_callback
};
use crate::features::kanban::hooks::use_task_sidebar;
use std::rc::Rc;
use std::sync::Arc;

#[component]
pub fn KanbanPage(project_id: String) -> impl IntoView {
        ...
        >
            <div class="main-content">
                <KanbanHeader
                    ...
                    on_back=Rc::new({
                        let navigate = use_context::<WriteSignal<crate::app::AppView>>()
                            .expect("AppView context should be available");
                        move || navigate.set(crate::app::AppView::Projects)
                    })
            ...
            // Conditional Sidebar using hook
            {task_sidebar}
            ...
            {move || {
                if let Some(task) = editing_task.get() {
                    let edit_callback = super::super::hooks::create_edit_task_callback(project_id.clone(), tasks);
                    view! {
                        <EditTaskModal
                            task=task
